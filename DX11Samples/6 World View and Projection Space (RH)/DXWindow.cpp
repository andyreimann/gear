...
DXWindow::initD3D()
{
	...
	// SET RENDER TARGET
	{
		...
		hr = mDevice->CreateDepthStencilState(&depthStencilStateDesc, &mDepthStencilState);
		if (SUCCEEDED(hr))
		{
...
DXWindow::initGraphics()
{
	...
	// SET TRANSFORMATIONS
	{
		...
		hr = mDevice->CreateBuffer(&cbbd, NULL, &cbPerObjectBuffer);
#endif

		//Set the View matrix
		///////////////**************mod**************////////////////////
		D3DXMatrixLookAtRH(&mCamView, &D3DXVECTOR3(0.0f, 0.0f, 0.5f), &D3DXVECTOR3(0.0f, 0.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));

		glm::mat4 camViewTmp;
		for (int i = 0; i < 16; ++i)
		{
			float* t = glm::value_ptr(camViewTmp);
			t[i] = mCamView[i];
		}
		std::cout << "BEFORE: " << std::endl;
		print(camViewTmp);

		// See http://www.gamedev.net/topic/508327-how-to-set-directx-c-on-a-right-handed-coordinate-system-by-default/
		mCamView._31 *= -1.f;
		mCamView._32 *= -1.f;
		mCamView._33 *= -1.f;
		mCamView._34 *= -1.f; 
		...
		D3DXMatrixPerspectiveFovRH(&mCamProjection, 0.4f*3.14f, (float)mWidth / (float)mHeight, 0.1f, 1000.0f);

		for (int i = 0; i < 16; ++i)
		{
			float* t = glm::value_ptr(camViewTmp);
			t[i] = mCamProjection[i];
		}
...
DXWindow::startFrame()
{
	...
	// DRAWING
	{
		...
		// transpose is only needed for RH Coordinate Systems
		...
		// Write the normal matrix into the shader -> RH Coordinate System!
		cgSetMatrixParameterfc(location, (float*)WVP);
