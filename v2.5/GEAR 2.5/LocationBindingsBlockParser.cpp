...
LocationBindingsBlockParser::LocationBindingsBlockParser(UberShader* uberShader, FileResource* file) 
...
LocationBindingsBlockParser::parse() 
{
	...
	if(mFile == nullptr || mUberShader == nullptr)
	{
		...
		logger << "[LocationBindingsBlockParser] -> Error 1001: given filehandle or UberShader is 0\n";
		return;
	}
	...
	if(mFile->isOpen()) 
	{
		while(!mFile->eof()) 
		{
			...
			mUberShader->add(LocationBinding(parsedSemantic, name, niceName, datatype));
		}
	}
}