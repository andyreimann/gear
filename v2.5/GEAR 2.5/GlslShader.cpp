...
GlslShader::compile(std::string const& vertexCode, std::string const& fragmentCode) 
{
	...
	{
		logger << "[GlslShader] Error: Compilation of Vertex Shader failed: " << errorLog;
		return mCompiled;
	}
	...
	if(errorLog.length() > 0) 
	{
		logger << "[GlslShader] Error: Compilation of Fragment Shader failed: %s" << errorLog;
		return mCompiled;
	}
	...
	if(errorLog.length() > 0) 
	{
		logger << "[GlslShader] Error: Shader linker failed: %s" << errorLog;
		return mCompiled;
	}
...
GlslShader::getAndCacheUniformLocation(std::string const& name) 
{
	...
	//qDebug("[GlslShader] Cache uniform '%s' to location %i", name.c_str(), location);
