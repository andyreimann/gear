...
NormalMapGenerator::generateFromHeightMap(unsigned char* heightMap, int size, float maxHeight) 
{
	...
	return std::shared_ptr<G2::Texture2D>(new Texture2D(GL_NEAREST, GL_NEAREST, size, size, GL_RGB, G2::WrapMode::CLAMP_TO_EDGE, G2::WrapMode::CLAMP_TO_EDGE, false, &normals[0]));			// The height map to use for this terrain
}