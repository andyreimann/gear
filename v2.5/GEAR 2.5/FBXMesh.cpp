...
FBXMesh::Builder::buildResource() 
{
	...
	auto* renderComponent = mesh->addComponent<RenderComponent>((unsigned int)meshMetaData.size());

	// init the VAO
	for (int i = 0; i < meshMetaData.size() ; ++i) 
	{
		MeshMetaData const& meshData = meshMetaData[i];
		renderComponent->vaos[i].resize((unsigned int)meshData.vertices.size())
			.writeData(Semantics::POSITION, &meshData.vertices[0]);
		
		if(meshData.hasNormals)
		{
			renderComponent->vaos[i].writeData(Semantics::NORMAL, &meshData.normals[0]);
		}
		if(meshData.hasUvs)
		{
			renderComponent->vaos[i].writeData(Semantics::TEXCOORD_0, &meshData.uvs[0]);
		}
		if(meshData.indices.size() > 0)
		{
			renderComponent->vaos[i].writeIndices(&meshData.indices[0], (unsigned int)meshData.indices.size());
		}
	}
